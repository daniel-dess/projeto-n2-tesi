...
class Tela():
    ...
    def main_inicio(self):
        ...
        def contar_questoes_cadastradas():
            sql = "SELECT * FROM questao;"
            resultado = listar(sql)
            return len(resultado)

        def contar_simulados_resolvidos():
            sql = "SELECT * FROM simulado;"
            resultado = listar(sql)
            return len(resultado)
        ...
        self.frm_inicio.grid_columnconfigure(0, weight=1)
        self.frm_inicio.grid_columnconfigure(1, weight=1)
        self.frm_inicio.grid_columnconfigure(2, weight=1)
        self.frm_inicio.grid_rowconfigure(0, weight=1)
        self.frm_inicio.grid_rowconfigure(1, weight=1)

        # Adicionando uma frame para os números de registros
        frm_numero0 = ttk.Frame(self.frm_inicio)
        frm_numero0.grid(row=0, column=0)

        frm_numero1 = ttk.Frame(self.frm_inicio)
        frm_numero1.grid(row=0, column=1)

        frm_numero2 = ttk.Frame(self.frm_inicio)
        frm_numero2.grid(row=0, column=2)

        questoes_cadastradas = contar_questoes_cadastradas()
        lbl_questoes_num = ttk.Label(frm_numero0, text=questoes_cadastradas, font=('Times New Roman', 16))
        lbl_questoes_num.grid(row=0, column=0)

        simulados_resolvidos = contar_simulados_resolvidos()
        lbl_simulados_num = ttk.Label(frm_numero1, text=simulados_resolvidos, font=('Times New Roman', 16))
        lbl_simulados_num.grid(row=0, column=1)

        usuarios_cadastrados = contar_usuarios_cadastrados()
        lbl_usuarios_num = ttk.Label(frm_numero2, text=usuarios_cadastrados, font=('Times New Roman', 16))
        lbl_usuarios_num.grid(row=0, column=2)

        # Adicionando os títulos
        lbl_questoes = ttk.Label(frm_numero0, text='Questões cadastradas', font=('Times New Roman', 16))
        lbl_questoes.grid(row=1, column=0)

        lbl_simulados = ttk.Label(frm_numero1, text='Simulados resolvidos', font=('Times New Roman', 16))
        lbl_simulados.grid(row=1, column=1)

        lbl_usuarios = ttk.Label(frm_numero2, text='Usuários cadastrados', font=('Times New Roman', 16))
        lbl_usuarios.grid(row=1, column=2)
